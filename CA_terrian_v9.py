...
# --- Load LLM JSON (update path if needed) ---
...
if terrain_tokens:
    base_terrain = terrain_counter.most_common(1)[0][0]
else:
    base_terrain = "grass"
    print("⚠️ No valid base terrain found. Defaulting to 'grass'.")
...
# --- Visualization with RANDOM color ---
def random_color():
    return (random.random(), random.random(), random.random())

terrain_color = random_color()
cmap = plt.matplotlib.colors.ListedColormap(["white", terrain_color])
...
legend_elements = [
    Patch(facecolor=terrain_color, edgecolor='black', label=f"{TILE_TERRAIN}: {base_terrain.title()}")
]
