...
json_path = "StoryFiles/2_object_affordance_langchain.json"  # update if using a different file
...
# ----------------- Visualize -----------------
scene_count = min(3, len(data["per_scene_affordances"]))
fig, axs = plt.subplots(scene_count, 3, figsize=(15, 4 * scene_count))
...
for idx in range(scene_count):
    ...
    vmin = 0
    vmax = max(tile_label_map.keys())

    axs[idx, 0].imshow(base, cmap=cmap, vmin=vmin, vmax=vmax)
    axs[idx, 0].set_title(f"Scene {idx+1} - Base\n({base_terrain})")
    axs[idx, 0].axis("off")

    axs[idx, 1].imshow(patch, cmap=cmap, vmin=vmin, vmax=vmax)
    axs[idx, 1].set_title(f"Scene {idx+1} - Patch\n({tile_label_map.get(patch_val, '')})")
    axs[idx, 1].axis("off")

    axs[idx, 2].imshow(combined, cmap=cmap, vmin=vmin, vmax=vmax)
    axs[idx, 2].set_title(f"Scene {idx+1} - Combined")
    axs[idx, 2].axis("off")
