...
TILESET_FOLDER = "Data/GameTile/sub_small_Tilesets"
SPLIT_TILE_FOLDER = "Data/GameTile/small_dataset"
TILE_SIZE = 32
THRESHOLD = 0.85
TRANSPARENCY_THRESHOLD = 0.5  # Adjust threshold for transparency detection
EDGE_CHECK_ROWS = 4  # Number of pixels to check for transparency at edges
...
def check_edge_transparency(tile):
    ...
    # Define tile regions based on your provided coordinates
    edge_transparency["top_left"] = is_edge_section_transparent(tile[0: TILE_SIZE // 2, 0:EDGE_CHECK_ROWS])
    edge_transparency["top_right"] = is_edge_section_transparent(tile[TILE_SIZE // 2: TILE_SIZE, 0:EDGE_CHECK_ROWS])
    
    edge_transparency["down_left"] = is_edge_section_transparent(tile[0: TILE_SIZE // 2, TILE_SIZE - EDGE_CHECK_ROWS:TILE_SIZE])
    edge_transparency["down_right"] = is_edge_section_transparent(tile[TILE_SIZE // 2: TILE_SIZE, TILE_SIZE - EDGE_CHECK_ROWS:TILE_SIZE])
    
    edge_transparency["left_top"] = is_edge_section_transparent(tile[0:EDGE_CHECK_ROWS, 0: TILE_SIZE // 2])
    edge_transparency["left_down"] = is_edge_section_transparent(tile[0:EDGE_CHECK_ROWS, TILE_SIZE // 2: TILE_SIZE])
    
    edge_transparency["right_top"] = is_edge_section_transparent(tile[TILE_SIZE - EDGE_CHECK_ROWS:TILE_SIZE, 0: TILE_SIZE // 2])
    edge_transparency["right_down"] = is_edge_section_transparent(tile[TILE_SIZE - EDGE_CHECK_ROWS:TILE_SIZE, TILE_SIZE // 2: TILE_SIZE])

    return edge_transparency
...
for tileset in tileset_images:
    ...
    for x, y in tile_coords:
        ...
        connectivity_list = []
        edge_transparency = check_edge_transparency(tile)  # Check transparency
...
# Save results
# output_file = "tile_connectivity_analysis_2_transparent.json"
# with open(output_file, "w") as json_file:
#     json.dump(results, json_file, indent=4)

# print(f"Connectivity analysis saved to {output_file}")

### explicitly convert NumPy booleans to Python booleans (True/False) before saving the JSON
output_file = "tile_connectivity_analysis_2_transparent.json"
with open(output_file, "w") as json_file:
