...
# --- GLOBAL PATCH SIZE CONTROL ---
MIN_PATCH_SIZE = 150
PATCH_SIZE_DELTA = 10  # final patch size will be random between MIN and MIN+DELTA
...
for obj_name, tile_id in patch_tile_ids.items():
    patch_size = (MIN_PATCH_SIZE, MIN_PATCH_SIZE + PATCH_SIZE_DELTA)
    place_patch(patch_layer, base_layer, tile_id, patch_size=patch_size)

visual_map = np.where(patch_layer != TILE_EMPTY, patch_layer, base_layer)

# --- VISUALIZATION ---
color_palette = [(1, 1, 1)]  # empty
...
plt.title(f"Base: '{base_terrain}' | Patches (min {MIN_PATCH_SIZE} tiles)")
plt.axis("off")
